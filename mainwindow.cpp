...
bool MainWindow::FOE(QString slave, eFOEDirection::E mode, QString filePath, QString password)
{
    ...
    {
        report("ERROR UPLOAD #" + QString::number(res) + " " + QString::number(out.usErrorID-65536));
        MMC_RESET_IN inr;
        MMC_RESET_OUT outr;
        inr.ucExecute=1;
    ...
    while (true)
    {
        ...
        {
            report("FOE STATUS: ERROR " + QString::number(outs.usErrorID));
            return false;
        }
        else
        {
            report("FOE STATUS: " + QString::number(outs.sFOEStatus) + " FOE PROGRESS : " +
...
bool MainWindow::SDOSendFile(QString slave, eSDODirection::E mode, QString filePath, QString password, tMD5 md5)
{
    bool ok(0);
    int pass=password.toUInt(&ok,16);
    qInfo()<<"pass:" << pass;
    ...
    fileHeader.setMD5(md5);
    Data=fileHeader.byteArray() + Data;
    ...
    if (fname.size()>11)
    {
        report("File name too long! " + QString::number(fname.size()) + " bytes!");
        return false;
    }
    ...
    memcpy(udata.pData+16, &dataSize, 4);
...
bool MainWindow::SDOReadFile(QString slave, eSDODirection::E mode, QString filePath, QString password, tMD5 md5)
{
...
void MainWindow::sscReadyReadStandardError()
{
    ...
    report("OUT: <" + StdOut + "> ERR: <" + StdError+">");
}
...
void MainWindow::sscErrorOccurred(QProcess::ProcessError error)
{
    QString StdOut = proc.readAllStandardOutput();  //Reads standard output
    QString StdError = proc.readAllStandardError(); // Reads standard error
    report("OUT: <" + StdOut + "> ERR: <" + StdError + "> err: <" + QString::number(error)+">");
}
...
bool MainWindow::SDOSendData(QString slave, eSDODirection::E mode, QString filePath, QByteArray Data, QString password)
{
    bool ok(0);
    int pass=password.toUInt(&ok,16);
    qInfo()<<"pass:" << pass;
...
void MainWindow::SDOSendMD5(const QString& slave, const QString& filePath, const QString& pass)
{
    ...
    QString md(md5s(Data.toStdString()).c_str());
    report(fi.completeBaseName()+" MD5 = " + md );
    SDOSendData(slave, eSDODirection::WRITE, fnamemd5,
                QByteArray::fromRawData(md.toStdString().c_str(), md.size()), pass);
}

bool MainWindow::md5FromFile(const QString& path, tMD5& arr)
{
...
void MainWindow::on_bSDOToSlave_2_clicked()
{
    ...
    if (ui->cbSDOToSlaveMD5_2->isChecked())
        md5FromFile(ui->eSDOFileNameTo_2->text(), md5);
    if (SDOSendFile(ui->eSlaveName->currentText(),eSDODirection::WRITE, ui->eSDOFileNameTo_2->text(), ui->eSDOPasswordTo_2->text(), md5))
    {
        /*if (ui->cbSDOToSlaveMD5_2->isChecked())
        {
...
void MainWindow::on_bSDOToSlave_3_clicked()
{
    ...
    if (ui->cbSDOToSlaveMD5_3->isChecked())
        md5FromFile(ui->eSDOFileNameTo_3->text(), md5);
    if (SDOSendFile(ui->eSlaveName->currentText(),eSDODirection::WRITE, ui->eSDOFileNameTo_3->text(), ui->eSDOPasswordTo_3->text(), md5))
    {
        /*if (ui->cbSDOToSlaveMD5_3->isChecked())
        {
...
void MainWindow::on_bSDOToSlave_4_clicked()
{
    ...
    if (ui->cbSDOToSlaveMD5_4->isChecked())
        md5FromFile(ui->eSDOFileNameTo_4->text(), md5);
    if (SDOSendFile(ui->eSlaveName->currentText(),eSDODirection::WRITE, ui->eSDOFileNameTo_4->text(), ui->eSDOPasswordTo_4->text(), md5))
    {
        /*if (ui->cbSDOToSlaveMD5_4->isChecked())
        {
...
void MainWindow::on_bSDOToSlave_5_clicked()
{
    ...
    if (ui->cbSDOToSlaveMD5_5->isChecked())
        md5FromFile(ui->eSDOFileNameTo_5->text(), md5);
    if (SDOSendFile(ui->eSlaveName->currentText(),eSDODirection::WRITE, ui->eSDOFileNameTo_5->text(), ui->eSDOPasswordTo_5->text(), md5))
    {
        /*if (ui->cbSDOToSlaveMD5_5->isChecked())
        {
...
void MainWindow::on_bSDOFromSlave_clicked()
{
    ...
    if (SDOReadFile(ui->eSlaveName->currentText(), eSDODirection::READ, filename, ui->eSDOPasswordFrom->text(), md5))
    {
        fileToMemo(filename);
    }
}
...
void MainWindow::on_bSDOFromSlave_2_clicked()
{
    ...
    tMD5 md5={0};
    md5FromFile(filename, md5);
    if (SDOReadFile(ui->eSlaveName->currentText(),eSDODirection::READ, filename, ui->eSDOPasswordFrom_2->text(), md5))
    {
        fileToMemo(filename);
    }
}
...
void MainWindow::on_bSDOFromSlave_3_clicked()
{
    ...
    tMD5 md5={0};
    md5FromFile(filename, md5);
    if (SDOReadFile(ui->eSlaveName->currentText(),eSDODirection::READ, filename, ui->eSDOPasswordFrom_3->text(), md5))
    {
        fileToMemo(filename);
    }
}
...
void MainWindow::on_bSDOFromSlave_4_clicked()
{
    ...
    tMD5 md5={0};
    md5FromFile(filename, md5);
    if (SDOReadFile(ui->eSlaveName->currentText(),eSDODirection::READ, filename, ui->eSDOPasswordFrom_4->text(), md5))
    {
        fileToMemo(filename);
    }
}
...
void MainWindow::on_bSDOFromSlave_5_clicked()
{
    ...
    tMD5 md5={0};
    md5FromFile(filename, md5);
    if (SDOReadFile(ui->eSlaveName->currentText(),eSDODirection::READ, filename, ui->eSDOPasswordFrom_5->text(), md5))
    {
        fileToMemo(filename);
    }
}
...
void MainWindow::on_bSDOFromSlaveMD5_clicked()
{
    m_action=eActions::aSDORead;
    QSettings settings;
    settings.beginGroup("SDO Parameters");
...
void MainWindow::on_bSDOFromSlaveMD5_2_clicked()
{
    m_action=eActions::aSDORead;
    QSettings settings;
    settings.beginGroup("SDO Parameters");
    settings.setValue("File name from",ui->eSDOFileNameFrom_2->text());
    settings.setValue("Password from",ui->eSDOPasswordFrom_2->text());
    settings.endGroup();
...
void MainWindow::on_bSDOFromSlaveMD5_3_clicked()
{
    m_action=eActions::aSDORead;
    QSettings settings;
    settings.beginGroup("SDO Parameters");
    settings.setValue("File name from",ui->eSDOFileNameFrom_3->text());
    settings.setValue("Password from",ui->eSDOPasswordFrom_3->text());
    settings.endGroup();
...
void MainWindow::on_bSDOFromSlaveMD5_4_clicked()
{
    m_action=eActions::aSDORead;
    QSettings settings;
    settings.beginGroup("SDO Parameters");
    settings.setValue("File name from",ui->eSDOFileNameFrom_4->text());
    settings.setValue("Password from",ui->eSDOPasswordFrom_4->text());
    settings.endGroup();
...
void MainWindow::on_bSDOFromSlaveMD5_5_clicked()
{
    m_action=eActions::aSDORead;
    QSettings settings;
    settings.beginGroup("SDO Parameters");
    settings.setValue("File name from",ui->eSDOFileNameFrom_5->text());
    settings.setValue("Password from",ui->eSDOPasswordFrom_5->text());
    settings.endGroup();
